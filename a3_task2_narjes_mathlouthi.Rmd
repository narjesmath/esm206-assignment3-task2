...
#convert `YYYYMMDD` to date class
ymd(20230322)

#convert `YYYY-MM-DD` to date class
ymd("2023-03-22")
...
gasol_hookshots <- lakers %>% 
  filter( player == "Pau Gasol", type == "hook",na.rm = TRUE) %>% 
   mutate(new_date = ymd(date)) %>% 
   mutate(new_date_month = month(new_date)) 
  

gasol_recorded_hookshots <- gasol_hookshots %>% 
  group_by(new_date_month) %>% 
  count() %>% 
  setNames(c('Month','Hook Shots'))


# View it: 
gasol_recorded_hookshots
